...
const isChildSelected = function (parent) {
    ...
    if ('tileid' in parent) {
        ...
        children = ko
            .unwrap(parent.cards)
            .concat(ko.unwrap(parent.tiles), ko.unwrap(parent.widgets));
    }
...
const doesChildHaveProvisionalEdits = function (parent) {
    ...
    ko.unwrap(parent[childrenKey]).forEach(function (child) {
        if (
            (child.hasprovisionaledits && child.hasprovisionaledits()) ||
            doesChildHaveProvisionalEdits(child)
        ) {
            hasEdits = true;
        }
    });
    ...
    $.get(
        ...
        function (descriptors) {
            if (typeof descriptors.displayname == 'string') {
                ...
                displayname(
                    descriptors.displayname.find(
                        (displayname) =>
                            displayname.language == arches.activeLanguage,
                    )?.value,
                );
            }
        },
...
const CardViewModel = function (params) {
    ...
    var nodegroup = _.find(ko.unwrap(nodegroups), function (group) {
        return (
            ko.unwrap(group.nodegroupid) === ko.unwrap(params.card.nodegroup_id)
        );
    });
    ...
    _.extend(this, nodegroup, {
        ...
        fullyProvisional: ko.pureComputed(function () {
            ...
            var summary = _.map(this.tiles(), function (tile) {
                ...
                if (ko.unwrap(tile.provisionaledits) !== null && dataEmpty) {
                    ...
                    ko.unwrap(tile.provisionaledits) !== null &&
                    !dataEmpty
                ) {
                    return 1;
                } else {
            ...
            provisionalindex = _.reduce(summary, function (a, b) {
                return a + b;
            });
            if (provisionalindex > 0) {
                if (
                    _.every(summary, function (val) {
                        return val === 2;
                    })
                ) {
        ...
        tiles: ko.observableArray(
            _.filter(params.tiles, function (tile) {
                return (
                    (params.tile
                        ? tile.parenttile_id === params.tile.tileid
                        : true) &&
                    ko.unwrap(tile.nodegroup_id) ===
                        ko.unwrap(params.card.nodegroup_id)
                );
            }).map(function (tile) {
        ...
        cards: ko.observableArray(
            _.filter(params.cards, function (card) {
                var nodegroup = _.find(ko.unwrap(nodegroups), function (group) {
                    return (
                        ko.unwrap(group.nodegroupid) ===
                        ko.unwrap(card.nodegroup_id)
                    );
                });

                if (nodegroup) {
                    return (
                        ko.unwrap(nodegroup.parentnodegroup_id) ===
                        ko.unwrap(params.card.nodegroup_id)
                    );
                }
            }).map(function (card) {
        ...
        showSummary: ko.pureComputed(function () {
            return (
                self.canAdd() && self.showForm() === false && self.selected()
            );
        }),
        ...
        getNewTile: function (forceNewTile) {
            if (!this.newTile || forceNewTile)
                this.newTile = new TileViewModel({
                    tile: {
                        tileid: '',
        ...
        isFuncNode: function () {
            var primaryDescriptorNodes = {},
                pdFunction = params.primaryDescriptorFunction;

            if (!pdFunction || !params.card) return false;

            ['name', 'description'].forEach(function (descriptor) {
                try {
                    primaryDescriptorNodes[
                        pdFunction['config']['descriptor_types'][descriptor][
                            'nodegroup_id'
                        ]
                    ] = descriptor;
                } catch (e) {
                    ...
                    console.log(
                        'No descriptor configuration for ' + descriptor,
                    );
                }
            });

            return !primaryDescriptorNodes[params.card.nodegroup_id]
                ? false
    ...
    this.copyTile = function (tile) {
        ...
        self.showForm(true);
        ko.mapping.fromJS(ko.mapping.toJS(tile.data), newTile.data);
    };
