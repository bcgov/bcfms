...
export default function (params) {
    ...
    this.getAllTiles = function (card) {
        return _.flatten([
            ko.unwrap(card.tiles),
            _.map(card.cards(), (subcard) => {
                return self.getAllTiles(subcard);
            }),
        ]);
    };
    ...
    var getValueFromTile = function (tile, widget) {
        return !tile || !widget
            ? null
            : ko.unwrap(tile.data[widget.node.nodeid]);
    };
    ...
    var getNodeValues = function (node_alias) {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            ...
            if (value === false || !!value) values.push(value);
        });
        return values;
    };
    ...
    this.scientificNames = ko.computed(function () {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            ...
            if (!!sciVal || !!connVal || !!otherNameVal)
                values.push({
                    scientific_name: ko.observable(sciVal),
    ...
    this.commonNames = ko.computed(function () {
        ...
        _.each(ko.unwrap(tiles), (tile) => {
            ...
            if (!!commonVal)
                values.push({
                    fossil_common_name: ko.observable(commonVal),
    ...
    this.getFossilNamesForCollectionEvent = ko.computed(function () {
        ...
        if (
            self.report.graph.slug === 'collection_event' &&
            self.tiles().length > 0
        ) {
    ...
    this.fossilNamesForCollectionEvent =
        self.getFossilNamesForCollectionEvent();
}
