...
const WidgetViewModel = function (params) {
    this.externalObservables = [
        'value',
        'config',
        'expanded',
        'defaultValueSubscription',
        'valueSubscription',
    ];
    var self = this;
    ...
    this.inResourceEditor =
        typeof params.inResourceEditor === 'boolean'
            ? params.inResourceEditor
            : null;
    ...
    if (ko.isObservable(this.value) && ko.isObservable(this.defaultValue)) {
        ...
        if (!self.form) {
            if (ko.isObservable(self.value)) {
                ...
                self.defaultValueSubscription = self.defaultValue.subscribe(
                    function (val) {
                        if (self.value() != val) {
                            self.value(val);
                        }
    ...
    this.valueProperties.forEach(function (property) {
        if (ko.isObservable(self.value)) {
            ...
            self[property].subscribe(function () {
                self.value(
                    self.valueProperties.reduce(function (data, property) {
