...
const CardModel = AbstractModel.extend({
    ...
    initialize: function (attributes) {
        ...
        var componentIdSubscription = this.get('component_id').subscribe(
            function (value) {
                var key;
                var defaultConfig =
                    self.cardComponentLookup[value].defaultconfig;
        ...
        this.dirty = ko.computed(function () {
            return (
                JSON.stringify(
                    _.extend(JSON.parse(self._card()), self.toJSON()),
                ) !== self._card()
            );
        });
        ...
        var widgetSubscription = this.get('widgets').subscribe(
            function (widgets) {
    ...
    parseNodes: function () {
        ...
        var widgetNodeIds = ko
            .unwrap(this.get('widgets'))
            .map(function (widget) {
        ...
        ko.unwrap(this.nodes).forEach(function (node) {
            ...
            var nodegroupId =
                ko.unwrap(node.nodeGroupId) || ko.unwrap(node.nodegroup_id);

            if (
                nodegroupId === ko.unwrap(attributes.data.nodegroup_id) &&
                !widgetNodeIds.includes(node.nodeid)
            ) {
                var datatype =
                    attributes.datatypelookup[ko.unwrap(node.datatype)];

                var nodeDatatypeSubscription = node.datatype.subscribe(
                    function () {
                        this._card(JSON.stringify(this.toJSON()));
                    },
                    this,
                ...
                if (datatype.defaultwidget_id) {
                    var cardWidgetData = _.find(
                        ko.unwrap(attributes.data.widgets),
                        function (widget) {
        ...
        var widgetsToDelete = ko.unwrap(this.get('widgets')).filter(function (
            widget,
        ) {
            var widgetNodegroupId =
                ko.unwrap(widget.node.nodeGroupId) ||
                ko.unwrap(widget.node.nodegroup_id);

            if (
                ko.unwrap(this.nodegroup_id) !== widgetNodegroupId ||
                !nodeIds.includes(widget.node_id())
            ) {
                return widget;
            }
        }, this);
    ...
    reset: function () {
        ...
        this.get('widgets')().forEach(function (widget) {
            var originalWidgetData = _.find(
                this._attributes.data.widgets,
                function (origwidget) {
            ...
            if (originalWidgetData) {
                widget.configKeys().forEach(function (configKey) {
                    koMapping.fromJS(
                        originalWidgetData.config[configKey],
            ...
            if (
                ...
                if (configKeys.length > 0) {
                    ...
                    _.each(configKeys, function (configKey) {
                        config[configKey] = ko.unwrap(
                            self.get('config')[configKey],
                        );
                    });
                }
                ret[key] = config;
    ...
    save: function (callback) {
        return AbstractModel.prototype.save.call(
            this,
