...
const popupDataProvider = {
    layerConfigs: {
        'WHSE_MINERAL_TENURE.GEOL_BEDROCK_UNIT_POLY_SVW': {
            ...
            map_popup: [
                { key: 'STRATIGRAPHIC_NAME', title: 'Strat Name:' },
        ...
        'WHSE_TANTALIS.TA_PARK_ECORES_PA_SVW': {
            ...
            map_popup: [
                { key: 'PROTECTED_LANDS_DESIGNATION', title: 'Designation: ' },
        ...
        'WHSE_ADMIN_BOUNDARIES.CLAB_NATIONAL_PARKS': {
            ...
            map_popup: [
                { key: 'FRENCH_NAME', title: 'French Name: ' },
    ...
    isFeatureClickable: function (feature, map) {
        ...
        if (
            (typeof selectedTool !== 'undefined' && selectedTool !== null) ||
            (selectedFeatureIds && selectedFeatureIds.length)
        )
            if (
                typeof drawMode !== 'undefined' &&
                drawMode !== null &&
                drawMode !== '' &&
                drawMode !== 'filter_by_feature'
            )
                return false;
        if (feature.sourceLayer in popupDataProvider.layerConfigs) return true;
    ...
    isSelectableAsFilter: function (feature) {
        return (
            popupDataProvider.filterLayers.indexOf(feature.sourceLayer) !== -1
        );
    },
    processData: function (dataFeatures) {
        ...
        dataFeatures.popupFeatures.forEach((featureData) => {
            if (
                ...
                featureData.feature.sourceLayer in this.layerConfigs
            ) {
                featureData.displayname = this.getDisplayValue(
                    featureData,
                    this.layerConfigs[featureData.feature.sourceLayer]
                        .displayname,
                );
            }
            if (
                !featureData.map_popup &&
                featureData.feature.sourceLayer in this.layerConfigs
            ) {
                featureData.map_popup = this.getDisplayValue(
                    featureData,
                    this.layerConfigs[featureData.feature.sourceLayer]
                        .map_popup,
                );
            ...
            featureData.showExpandButton = function (container, event) {
                ...
                if (div.length > 0) {
                    return (
                        div[0].offsetWidth < div[0].scrollWidth ||
    ...
    sendFeatureToMapFilter: function (popupFeatureObject, addToFilter) {
        const foundFeature = popupFeatureObject.feature.properties.featureid
            ? this.findPopupFeatureById(popupFeatureObject)
            : mapFilterUtils.getFeatureFromWFS(
                  popupFeatureObject.feature,
                  popupFeatureObject.feature.sourceLayer,
              );
