...
def generate_filename(instance, filename):
    ...
    return os.path.join(
        str(instance.tile.resourceinstance.graph.slug), *paths, filename
    )
...
def get_publication_filename(instance, filename):
    ...
    if not hasattr(get_publication_filename, "publication_year_node"):
        get_publication_filename.publication_year_node = models.Node.objects.filter(
    ...
    if not hasattr(get_publication_filename, "publication_type_node"):
        get_publication_filename.publication_type_node = models.Node.objects.filter(
            alias="publication_type"
        ).first()

    if not hasattr(get_publication_filename, "publication_year_datatype"):
        get_publication_filename.publication_year_datatype = (
    ...
    if not hasattr(get_publication_filename, "publication_type_datatype"):
        get_publication_filename.publication_type_datatype = (
            datatypes.datatypes.DataTypeFactory().get_instance(
                get_publication_filename.publication_type_node.datatype
            )
        )

    publication_tile = models.TileModel.objects.filter(
        resourceinstance=instance.tile.resourceinstance,
        nodegroup=get_publication_filename.publication_year_node.nodegroup,
    ).first()

    if publication_tile:
        publication_type = (
            get_publication_filename.publication_type_datatype.get_display_value(
                publication_tile, get_publication_filename.publication_type_node
            )
        )
        publication_year = (
            get_publication_filename.publication_year_datatype.get_display_value(
                publication_tile, get_publication_filename.publication_year_node
            )
        )
