...
class ScientificTermsFormatter:
    ...
    @staticmethod
    def format_scientific_name(
        scientific_name, open_nomenclature_term, other_scientific_name
    ):
        formatted_name = ""
        if (scientific_name is None or scientific_name == "") and (
            other_scientific_name is None or other_scientific_name == ""
        ):
            return None
        if scientific_name:
        ...
        if (
            scientific_name
            and not open_nomenclature_term
            and not other_scientific_name
            and not re.match(".* .*", scientific_name)
        ):
    ...
    def format_common_name(common_name, common_name_uncertain):
        return ScientificTermsFormatter._format_uncertain(
            common_name, common_name_uncertain
        )

    @staticmethod
    def format_times(min_time, min_time_uncertain, max_time, max_time_uncertain):
        min_time_str = ScientificTermsFormatter._format_uncertain(
            min_time, min_time_uncertain
        )
        max_time_str = ScientificTermsFormatter._format_uncertain(
            max_time, max_time_uncertain
        )
        ...
        return (
            "%s - %s"
            % (
                min_time_str if min_time_str else "",
                max_time_str if max_time_str else "",
            )
        ).rstrip()
